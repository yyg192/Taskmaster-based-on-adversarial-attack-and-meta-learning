...
class SimpleBA(AttackerBase):
    def __init__(self,
                 target_model,
                 epislon,
                 maximum_query_iterations=200):
        super(SimpleBA, self).__init__()
    ...
    def graph(self):
        ...
        def _cond(query_num, _attack_success, x_adv_tmp, last_prob):
            return tf.logical_and(
                tf.logical_not(_attack_success),
                tf.less(query_num, self.maximum_query_iterations)
            )
        ...
        def _body(query_num, _attack_success, x_adv_tmp, last_prob):
            perm = tf.random.uniform(shape=[self.n_dims, ], minval=0, maxval=self.n_dims, dtype=tf.int32)
            # perm.shape = (1024, )
            diff = tf.one_hot(perm[query_num], self.n_dims)
            # diff.shape = (1024, )
            diff = tf.multiply(diff, self.epislon)
            # diff.shape = (
            x1 = tf.subtract(x_adv_tmp, diff)
            ...
            # left_prob.shape=(?, )
            x2 = tf.add(x_adv_tmp, diff)
            x2_clip = tf.clip_by_value(x2,
                                       clip_value_min=scaled_min_extended,
                                       clip_value_max=scaled_max_extended)
            right_prob = self.target_model.get_soft_output_logits(x2_clip)[:, 1]
            x_adv_tmp, last_prob = tf.cond(tf.less(left_prob[0], last_prob[0]),
                                           lambda: f1(x1_clip, left_prob),
                                           lambda: f2(x2_clip, right_prob, x_adv_tmp, last_prob))
            _attack_success = tf.less(last_prob, tf.constant(0.5))[0]
            return query_num + 1, _attack_success, x_adv_tmp, last_prob

        last_prob_init = self.target_model.get_soft_output_logits(self.x_input)[:, 1]
        qn, attack_success, x_adv, lp = tf.while_loop(_cond,
                                                      _body,
                                                      (0, False, self.x_input, last_prob_init))
        return qn, attack_success, x_adv, lp
    """
    ...
    def simba_attack(self, malware_feature_vectors, epsilon=1., target_sess=None):
        query_num_total = []
...
if __name__ == "__main__":
    ...
    with target_graph.as_default():
        with target_sess.as_default():
            ...
            SBA = SimpleBA(target_model, 1, 500)
            SBA.simba_attack(malware_feature_vectors[:500], epsilon=1, target_sess=target_sess)
