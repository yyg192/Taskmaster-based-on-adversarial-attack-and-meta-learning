...
class BlackBoxAttackPGDL2(AttackerBase):
    def __init__(self,
                 target_model,
                 substitute_model,
                 ...
                 substitute_training_interval=5,
                 ):
        super(BlackBoxAttackPGDL2, self).__init__()
    ...
    def single_iteration(self):
        ...
        debug_info = perturbations
        return x_adv_out, tf.nn.softmax(logits), attack_success, debug_info
    ...
    def blackbox_attack(self, single_malware_vector,
        ...
        while query_num < self.maximum_query_iterations:
            ...
            x_adv, logits, attack_success, debug_info = \
                ...
                with target_sess.as_default():
                    if self.label_type is "soft_label":
                        target_logits = target_sess.run(self.target_model.soft_output_logits,
                                                        feed_dict={self.target_model.x_input: x_adv})
            ...
            if query_num == self.maximum_query_iterations:
                average_modified = np.sum(
                    np.abs(utils.map_to_discrete_domain(self.normalizer, x_adv)[0] - single_malware_vector[0]) > 1e-6)
            ...
            if query_num > self.warmup_iterations and query_num % self.substitute_training_interval == 0:
...
def attack():
    ...
    utils.dump_pickle(np.array(attack_success_record),
                      config.get("advtraining.drebin", "advtraining_drebin_root") + "/attack_success_record_pgdl2_adv3")
    utils.dump_pickle(np.array(query_num_record),
                      config.get("advtraining.drebin", "advtraining_drebin_root") + "/query_num_record_pgdl2_adv3")
    utils.dump_pickle(np.array(modified_nums_record),
                      config.get("advtraining.drebin", "advtraining_drebin_root") + "/modified_nums_record_pgdl2_adv3")
