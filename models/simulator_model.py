...
class SimulatorModel:
    def __init__(self,
        ...
        self.outter_update_iterations = 30
        """  学习率  """
        self.inner_update_learning_rate = 1e-6  # 试一下0.01
        self.outter_update_learning_rate = 1e-6
        ...
        if advtraining_method is not None and advtraining_number is not None and simulator_exp_name is not None:
            """ 模型保存地址 """
            self.simulator_model_save_dir = \
                config.get("DEFAULT", "adv_train_root") + \
                "/meta_simulator/" + simulator_exp_name + '/'+distillation_method+"/adv" + str(advtraining_number)
            ...
            """ tensorboard log地址 """
            self.tensorboard_log_dir = config.get("metatraining.drebin", "model_simulator_log_dir")+"/"+\
                simulator_exp_name + "/" + distillation_method + "/adv" + str(advtraining_number)
            print("tensorboard_logdir = ", self.tensorboard_log_dir)
    ...
    def build_regular_simulator(self):
        ...
        self.softmax_output_logits = tf.nn.softmax(self.regular_logits)
...
def _main():
    ...
    ori_benware_feature_vectors = read_pickle(config.get(benware_dataset_name, "sample_vectors"))
    ori_malware_feature_labels = to_categorical(np.ones(len(ori_malware_feature_vectors)), num_classes=2)
    ori_benware_feature_labels = to_categorical(np.ones(len(ori_benware_feature_vectors)), num_classes=2)
    ...
    ########################
    teachers_name = ['model_A', 'model_B', 'model_C', 'model_D', 'model_E', 'model_F', 'model_H']
    advtraining_method = "pgdl2"  # option: "pgdl2 pgdl1 pgdlinfinity jsma
    teacher_exp_name = "3_4L_nobalanced_nobenwaretrain_15_epochs_4096_" + malware_dataset_name + \
        "_AND_" + benware_dataset_name
    adv_train_root = config.get("advtraining.drebin",
                                "advtraining_drebin_root") + "/" + advtraining_method + "_" + teacher_exp_name
    advtraining_number = 6  # 对于pgdl2来说，经历过一次adv_training就够了。
    simulator_exp_name = "3_4L_nobalanced_nobenwaretrain_15_epochs_4096_inner10_1e-6_outter30_1e-6"
    ...
    for j, teacher_name in enumerate(teachers_name):
        ...
        with graph_j.as_default():
            with sess_j.as_default():
                ...
                teacher_model_load_dir = adv_train_root + "/" + teacher_name + "/adv" + str(advtraining_number)
                teacher_saver = tf.train.Saver()#tf.train.import_meta_graph(cur_checkpoint+".meta")
    ...
    with simulator_graph.as_default():
        with simulator_sess.as_default():
            simulator_model = SimulatorModel(advtraining_number=advtraining_number,
                                             advtraining_method=advtraining_method,
                                             distillation_method="pgdl2",
                                             simulator_exp_name=simulator_exp_name)
