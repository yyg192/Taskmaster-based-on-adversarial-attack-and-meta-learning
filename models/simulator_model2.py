...
class SimulatorModel:
    def __init__(self,
        ...
        self.outter_update_iterations = 50
        """  学习率  """
        ...
        if advtraining_method is not None and advtraining_number is not None and exp_name is not None:
            ...
            self.simulator_model_save_dir = \
                ...
                "/meta_simulator/" + exp_name + '/pgdl2' + "/adv" + str(advtraining_number)
            ...
            self.tensorboard_log_dir = \
                config.get("DEFAULT", "project_root") + "/tensorboard_log_dir" + "/" + exp_name + "/pgdl2/" + str(
                    advtraining_number)
...
def _main():
    ...
    advtraining_method = "pgdl1"  # option: "pgdl2 pgdl1 pgdlinfinity jsma
    ...
    adv_train_root = config.get("advtraining.drebin",
                                "advtraining_drebin_root") + "/" + advtraining_method + "_" + exp_name

    advtraining_number = 7  # 对于pgdl2来说，经历过一次adv_training就够了。
    ...
    for j, teacher_name in enumerate(teachers_name):
        ...
        with graph_j.as_default():
            with sess_j.as_default():
                ...
                cur_checkpoint = tf.train.latest_checkpoint(teacher_model_load_dir)
