...
class pgdl2_generator(AttackerBase):
    ...
    def graph_substitute(self):
        ...
        def _body(i, _attack_success, x_adv_tmp):
            ...
            x_adv_discrete = utils.map_to_discrete_domain_TF(self.normalizer, x_adv_tmp)
            predict_discrete = self.model.get_output_pred(x_adv_discrete)
            predict = self.model.get_output_pred(x_adv_tmp)
            _attack_success = \
                tf.logical_not(tf.equal(predict_discrete, tf.argmax(self.model.y_input, axis=-1, output_type=tf.int32)))[0]

            return i + 1, _attack_success, x_adv_tmp

        iter_num, attack_success, x_adv_var = tf.while_loop(_cond, _body, (0, False, self.model.x_input),
                                                            maximum_iterations=self.maximum_iterations)
        return iter_num, utils.map_to_discrete_domain_TF(self.normalizer, x_adv_var), attack_success
