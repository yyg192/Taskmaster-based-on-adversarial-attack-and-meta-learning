...
def optimize_linear(grad, eps, ord=np.inf):
    ...
    :param grad: tf tensor containing a batch of gradients 包含一批线性的张量
    :param eps: float scalar specifying size of constraint region  指定约束区域大小的浮点标量
    :param ord: int specifying order of norm         int指定规范顺序
    :returns:
    tf tensor containing optimal perturbation      tf张量包含最优扰动
    ...
    red_ind = list(range(1, len(grad.get_shape())))  #red_ind的意思是reduction_indices
    #print("red_ind",red_ind)                          red_ind [1]
    #print("red_ind.shape",np.array(red_ind).shape)    red_ind.shape (1,)
    avoid_zero_div = 1e-12
    if ord == np.inf:
        # Take sign of gradient
        optimal_perturbation = tf.sign(grad)   # optimal_perturbation.shape (?, 3974)
        ...
        """
        https://github.com/cleverhans-lab/cleverhans/blob/master/cleverhans_v3.1.0/cleverhans/attacks/fast_gradient_method.py
        终于找到出处了！！！
        但是论文来源我还没有找到!!! 
        """
        ...
        # https://blog.csdn.net/qq_33096883/article/details/77479766

        tied_for_max = tf.to_float(tf.equal(abs_grad, max_abs_grad))
        #这里是寻找abs_grad中梯度最大的位置，注意这个tied_for_max是True-False矩阵，只会指示位置。
        ...
        """
                ...
                [[1, 2, 3, 3, 1],
                 ...
                 [[2.]
                  [1.]
                  [1.]]
                 tied_for_max / num_ties = 
                 [[0.  0.  0.5 0.5 0. ]
                  [0.  1.  0.  0.  0. ]
                  [0.  0.  0.  1.  0. ]]
        """
        optimal_perturbation = sign * tied_for_max / num_ties
        #optimal_perturbation 要么就都是0，要么就是带符号的max值
        ...
        square = tf.maximum(avoid_zero_div,
                            tf.reduce_sum(tf.square(grad),
                                       ...
                                       keepdims=True))
        #这个tf.maximum的意思是比avoid_zero_div小的都元素都变成avoid_zero_div
    ...
    # norm=1 problem
    scaled_perturbation = tf.multiply(eps, optimal_perturbation) #这个就是扰动强度
...
def map_to_discrete_domain(normalizer, x_unnorm_input):
    """
    :param x_unnorm: continuous numpy array with shape [sample_nums, input_dim]
    :return: discrete numpy array with shape [sample_nums, input_dim]
    """
    x_unnorm = x_unnorm_input.copy()
    x_unnorm = np.divide(x_unnorm - normalizer.min_, normalizer.scale_)
    x_unnorm = np.rint(x_unnorm)
    x_unnorm = np.multiply(x_unnorm, normalizer.scale_) + normalizer.min_
    return x_unnorm

def map_to_discrete_domain_TF(normalizer, x_unnorm_input):
    """
    :param x_unnorm: continuous numpy array with shape [sample_nums, input_dim]
    :return: discrete numpy array with shape [sample_nums, input_dim]
    """
    x_unnorm = tf.divide(x_unnorm_input - normalizer.min_, normalizer.scale_)
    x_unnorm = tf.rint(x_unnorm)
    x_unnorm = tf.multiply(x_unnorm, normalizer.scale_) + normalizer.min_
    return x_unnorm
