...
class pgdl2_generator(AttackerBase):
    def __init__(self,
                 target_model,
                 maximum_iterations,
                 force_iteration,
                 ...
                 mask_rate,
                 step_size=10.):
        super(pgdl2_generator, self).__init__()
        ...
        self.launch_an_attack_simulator = self.graph_substitute()
    ...
    def graph_teacher(self):
        ...
        def _body(i, _attack_success, x_adv_tmp):
            loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits=self.model.get_output_logits(x_adv_tmp),
            ...
            _attack_success = tf.logical_not(tf.equal(predict, tf.argmax(self.model.y_input, axis=-1, output_type=tf.int32)))[0]
    ...
    def graph_substitute(self):
        ...
        _, attack_success, x_adv_var = tf.while_loop(_cond, _body, (0, False, self.model.x_input),
                                                     maximum_iterations=self.maximum_iterations)
        return x_adv_var, attack_success

    def genrate_attack_samples_teacher(self,
        ...
        if samples_save_dir is not None:
            ...
            utils.dump_pickle(x_adv_all, samples_save_dir + "/x_adv_all")
            utils.dump_pickle(x_ori, samples_save_dir + "/x_ori")
        return x_adv_success
...
def generate_attack_samples_substitute(self,
    ...
    for idx, x_input_var, y_input_var in input_data.next_batch():
        ...
        for _ in range(require_sample_nums):
            ...
            if self.random_mask is True:
                ...
                mask_arr = np.ones((1, self.input_dim))
            x_adv_var, _attack_success = sess.run(self.launch_an_attack_substitute,
                                                  feed_dict={self.model.x_input: x_input_var,
                                                             self.model.y_input: y_input_var,
    ...
    return x_adv_all, logits, visualization_info
